...
import { Phone, MessageCircle, Instagram, MapPin, Calendar, Clock, User, Mail, ChevronRight, Menu, X, Globe, Award, Users, Shield, Zap } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-12">
        ...
        <div className="flex-1 relative">
          ...
          <div className="relative p-2 bg-white rounded-[2rem] shadow-2xl border border-gray-100">
            <img 
              src="https://images.pexels.com/photos/6812559/pexels-photo-6812559.jpeg" 
              alt="Cabinet Dentaire MAPSO" 
...
const ServicesGrid: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  const handleBookService = (serviceTitle: string) => {
    const message = lang === 'AR' 
      ? `السلام عليكم MAPSO، أود حجز موعد ل${serviceTitle}`
      : `Bonjour MAPSO, je souhaite prendre un rendez-vous pour ${serviceTitle}`;
...
const Stats: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  const stats = [
    { label: t.stats_patients, value: "5000+", icon: <Users className="w-6 h-6" /> },
    { label: t.stats_experience, value: "15+", icon: <Clock className="w-6 h-6" /> },
    { label: t.stats_specialists, value: "10+", icon: <User className="w-6 h-6" /> },
    { label: t.stats_awards, value: "12", icon: <Award className="w-6 h-6" /> },
  ];

  return (
    <section className="py-12 bg-clinic-primary text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {stats.map((stat, idx) => (
            <div key={idx} className="text-center space-y-2 animate-fade-in">
              <div className="inline-flex p-3 bg-white/10 rounded-2xl text-clinic-accent mb-2">
                {stat.icon}
              </div>
...
const Features: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  const features = [
    { title: t.feature_1_title, desc: t.feature_1_desc, icon: <Shield className="w-8 h-8" /> },
    { title: t.feature_2_title, desc: t.feature_2_desc, icon: <Zap className="w-8 h-8" /> },
    { title: t.feature_3_title, desc: t.feature_3_desc, icon: <Clock className="w-8 h-8" /> },
    { title: t.feature_4_title, desc: t.feature_4_desc, icon: <Award className="w-8 h-8" /> },
  ];

  return (
    <section className={`py-24 px-4 sm:px-6 lg:px-8 bg-white ${isRtl ? 'rtl' : 'ltr'}`}>
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-20">
          <h2 className={`text-3xl md:text-4xl font-bold text-clinic-primary mb-4 ${isRtl ? 'font-arabic' : ''}`}>{t.features_title}</h2>
          <p className="text-gray-500 max-w-2xl mx-auto">{t.features_subtitle}</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          {features.map((f, idx) => (
            <div key={idx} className="p-8 bg-clinic-surface rounded-[2rem] border border-gray-100 hover:border-clinic-accent transition-all group">
              <div className="w-16 h-16 bg-white rounded-2xl shadow-sm flex items-center justify-center text-clinic-accent mb-6 group-hover:scale-110 transition-transform">
                {f.icon}
              </div>
              <h3 className={`text-xl font-bold text-clinic-primary mb-3 ${isRtl ? 'font-arabic' : ''}`}>{f.title}</h3>
